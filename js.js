document.addEventListener("DOMContentLoaded", () => {
				...
				switch (i) {
					...
					case 2:
						if (j === 0) {
							...
							showsettings
								? document.querySelectorAll(".buttoncontainer")[i].children[0].classList.add("green")
								: document.querySelectorAll(".buttoncontainer")[i].children[0].classList.remove("green");
						}
						if (j === 1) {
							showstatistics = !showstatistics;
							showstatistics
								? document.querySelectorAll(".buttoncontainer")[i].children[1].classList.add("green")
								: document.querySelectorAll(".buttoncontainer")[i].children[1].classList.remove("green");
						}
						break;
	...
	function restart() { //restart the game by redoing the audio
		context.close();
    ...
    function fade2(start, end, current, time) { //second portion of the fading animation
		...
		if (current <= end) { //once animation is finished, run process similar to restart, with some functionalities added
            ...
            gain.connect(context.destination);
            oscillator.start();
            for (let i = 0; i < coefficents.length; i++) { //fader creation
            ...
            [...document.querySelectorAll(".slider")].forEach((e, i) => { //fader functionality
			...
			document.getElementById("settings").innerHTML =
				"Oscillator: " +
				["Sine", "Sawtooth", "Square", "Triangle"][oscillatormode] +
				"<br>Mode: " +
	...
	function fade3(start, end, current, time) { // second part of the ending animation
		document.getElementById("fader").style.opacity = current;
		if (current <= end) { //reset all variables and some text
			document.getElementById("fader").style.display = "none";
    ...
    document.getElementById("play").addEventListener("click", () => { //play button
    ...
    document.getElementById("submit").addEventListener("click", () => {
        if (delay) {
			if (guess === answer) { //handle winning and losing
			...
			document.getElementById("statisticsdisplay").innerHTML = //statistic display on the left
				...
				(coefficents[answer] - coefficents[guess]).toString() + "Hz / " + (answer - guess).toString() + " bands away" +
				"<br>Guess: <br>" +
